...
# ==========================================================
# [2] 예약 기능 (프론트엔드 KaraokeAPI.enqueue와 연동)
# ==========================================================
...
async def enqueue_song(song_id: int, room_id: str = "Room_A", db: Session = Depends(get_db)):
    ...
    db.add(new_queue)
    db.commit()
    return {"message": f"'{song.title}' 예약 완료!", "position": next_pos}

# ==========================================================
# [3] 다음 곡 재생 (프론트엔드 handlePlayNext와 연동)
# ==========================================================
...
async def play_next(room_id: str = "Room_A", current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {
        "song_id": song.id,
        "title": song.title,
        "video_url": song.video_url, # 유튜브 연동 핵심
        "remaining_plays": remaining
    }

# ==========================================================
# [4] 가사 데이터 제공 (프론트엔드 startLyrics와 연동)
# ==========================================================
...
async def get_lyrics(song_id: int, db: Session = Depends(get_db)):
    ...
    # 사장님, 여기에 실제 가사 DB가 없다면 아래처럼 샘플 싱크를 내려줍니다.
    sample_sync = [
        {"time": 2, "text": "🎵 (전주 흐르는 중...)"},
        {"time": 5, "text": "첫 소절이 시작됩니다!"},
        {"time": 10, "text": "화면의 가사를 보고 따라 불러보세요!"},
        {"time": 15, "text": "마지막 소절입니다. 수고하셨습니다! 🎤"}
    ]
